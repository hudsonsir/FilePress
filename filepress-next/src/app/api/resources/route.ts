import { NextRequest } from "next/server";
import { getSession } from "@/lib/auth";
import { query, TABLE_PREFIX } from "@/lib/db";
import { apiSuccess, apiError, parseIntParam } from "@/lib/utils";
import type { RowDataPacket } from "mysql2";

export async function GET(req: NextRequest) {
  const user = await getSession();
  if (!user) return apiError("请先登录", 401, 401);

  const { searchParams } = new URL(req.url);
  const appid = searchParams.get("appid");
  const page = parseIntParam(searchParams.get("page"), 1);
  const pageSize = parseIntParam(searchParams.get("pageSize"), 60);
  const sort = searchParams.get("sort") || "btime";
  const order = searchParams.get("order") === "asc" ? "ASC" : "DESC";
  const keyword = searchParams.get("keyword") || "";
  const ext = searchParams.get("ext") || "";
  const fid = searchParams.get("fid") || "";
  const tagId = searchParams.get("tagId") || "";
  const type = searchParams.get("type") || "";

  if (!appid) return apiError("缺少 appid 参数");

  const allowedSort: Record<string, string> = {
    btime: "r.btime",
    mtime: "r.mtime",
    dateline: "r.dateline",
    name: "r.name",
    size: "r.size",
  };
  const sortCol = allowedSort[sort] ?? "r.btime";

  const conditions: string[] = [
    `r.appid = ?`,
    `r.isdelete = 0`,
  ];
  const values: unknown[] = [appid];

  if (keyword) {
    conditions.push(`(r.name LIKE ? OR ra.searchval LIKE ? OR ra.desc LIKE ?)`);
    const kw = `%${keyword}%`;
    values.push(kw, kw, kw);
  }

  if (ext) {
    conditions.push(`r.ext = ?`);
    values.push(ext.toLowerCase());
  }

  if (type) {
    const typeExtMap: Record<string, string[]> = {
      image: ["jpg","jpeg","png","gif","webp","svg","bmp","heic","tif","tiff","psd","ai","eps","raw"],
      video: ["mp4","avi","mov","wmv","flv","mkv","webm","m4v","ts","rmvb"],
      audio: ["mp3","wav","ogg","m4a","flac","aac","ape","aiff","amr"],
      document: ["pdf","doc","docx","xls","xlsx","ppt","pptx","txt","csv","md"],
    };
    const exts = typeExtMap[type];
    if (exts) {
      conditions.push(`r.ext IN (${exts.map(() => "?").join(",")})`);
      values.push(...exts);
    }
  }

  // 目录筛选
  let joinClause = `LEFT JOIN \`${TABLE_PREFIX}pichome_resources_attr\` ra ON ra.rid = r.rid`;
  if (fid) {
    joinClause += ` INNER JOIN \`${TABLE_PREFIX}pichome_folderresources\` fr ON fr.rid = r.rid AND fr.fid = ?`;
    values.push(fid);
  }

  // 标签筛选
  if (tagId) {
    joinClause += ` INNER JOIN \`${TABLE_PREFIX}pichome_resourcestag\` rt ON rt.rid = r.rid AND rt.tid = ?`;
    values.push(tagId);
  }

  const where = conditions.join(" AND ");

  // 计数
  const [[countRow]] = await (await import("@/lib/db")).default.execute<RowDataPacket[]>(
    `SELECT COUNT(DISTINCT r.rid) AS total
     FROM \`${TABLE_PREFIX}pichome_resources\` r
     ${joinClause}
     WHERE ${where}`,
    values
  );
  const total = (countRow as RowDataPacket).total as number;

  // 数据
  const offset = (page - 1) * pageSize;
  const rows = await query<RowDataPacket[]>(
    `SELECT r.rid, r.name, r.ext, r.type, r.size, r.width, r.height,
            r.btime, r.mtime, r.dateline, r.hasthumb, r.grade, r.appid,
            ra.desc, ra.duration, ra.colors, ra.shape, ra.link
     FROM \`${TABLE_PREFIX}pichome_resources\` r
     ${joinClause}
     WHERE ${where}
     GROUP BY r.rid
     ORDER BY ${sortCol} ${order}
     LIMIT ? OFFSET ?`,
    [...values, pageSize, offset]
  );

  return apiSuccess({ list: rows, total, page, pageSize });
}
